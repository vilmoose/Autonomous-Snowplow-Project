...
void setup() {
  ...
  */
  ...
  PWM->PWM_CH_NUM[0].PWM_CPRD = 700-1; //Set PWM freq = 1MHz/(FREQ) 
  ...
  PWM->PWM_CH_NUM[1].PWM_CMR |= PWM_CMR_CPRE_CLKA // Set the clock source as CLKA
                              | PWM_CMR_CPOL; //Set output polarity be high.
  PWM->PWM_CH_NUM[1].PWM_CPRD = 700-1; //Set PWM freq = 1MHz/(FREQ) 
...
void loop() {
  ...
  LFResponse();
  USTOFResponse();
   // analyzeData(); // do we even need this?
  delay(500);
 }
...
void analyzeData(){
  ...
  if(listOfAllSensors[1].state == true & listOfAllSensors[2].state == true){
    USTOFResponse();
  }
...
// _________________________________________________________ //
 ...
 void moveForward(){
  PWM->PWM_CH_NUM[0].PWM_CDTY = 800-1; // Set PWM duty cycle 70% (standard for now)
  PWM->PWM_CH_NUM[1].PWM_CDTY = 800-1; // Set PWM duty cycle 70% (standard for now)
  digitalWrite(45, LOW); 
  digitalWrite(47, LOW); 
...
void moveBackward(){
  PWM->PWM_CH_NUM[0].PWM_CDTY = 800-1; // Set PWM duty cycle 40% to go slower backwards
  PWM->PWM_CH_NUM[1].PWM_CDTY = 800-1; // Set PWM duty cycle 50% (standard for now)
...
void turnRight(){
  PWM->PWM_CH_NUM[0].PWM_CDTY = 400-1; 
  PWM->PWM_CH_NUM[1].PWM_CDTY = 900-1; 
  digitalWrite(45, LOW);
  digitalWrite(47, LOW); 
  digitalWrite(49, LOW); 
  digitalWrite(51, LOW); 
...
void turnLeft(){
  PWM->PWM_CH_NUM[0].PWM_CDTY = 900-1; 
  PWM->PWM_CH_NUM[1].PWM_CDTY = 400-1; 
  digitalWrite(45, HIGH);
  digitalWrite(47, HIGH); 
  digitalWrite(49, HIGH); 
  digitalWrite(51, HIGH); 
...
*/ 
 void LFResponse(){
  ...
  if(leftIn > 850 | middleIn > 850 | rightIn > 850){
    //stop();
    Serial.println("black surface detected");
  }  
 
  if(leftIn < 600 | middleIn < 600 | rightIn <600){
    //moveForward();
...
void USTOFResponse(){
  /**
  Here both the time of flight sensor and ultrasonic are used 
  for obstacle detection. The Ultrasonic sensor is there to account 
  for the blind spot of the time of flight but the time of flight also
  accounts for the failure of the ultrasonic.
  */
  ...
  int left1 = (sensors[0].read()/10.0);
  int right1 = (sensors[1].read()/10.0);
   //testing
  Serial.print("Left side(cm): "); //left side
  Serial.println(left1);
  if (sensors[0].timeoutOccurred()) { Serial.print(" TIMEOUT"); }
  Serial.print("Right side(cm): "); //right side
...
/**
  ...
  if (left1 < 12.0){
    //This instance is here incase the ultrasonic Sensor fails
    Serial.println("I'm turning LEFT by force");
    stop();
    delay(100);
    moveBackward();
    delay(1000);
    ...
    //This instance is here incase the ultrasonic Sensor fails
    Serial.println("I'm turning RIGHT by force");
    stop();
    delay(100);
    moveBackward();
    delay(1000);
  ...
  if (left1 < 30.0 & dist < 30.00){
    Serial.println("I'm turning LEFT");
    turnRight();
    delay(100);

  }
  else if(right1 < 30.0 & dist < 30.00){
