...
void setup() {
  ...
  */ 
  pinMode((uint32_t)45, OUTPUT);  //Motor A(1) = front left
  pinMode((uint32_t)47, OUTPUT); //Motor B(2) = back left
  pinMode((uint32_t)49, OUTPUT); //Motor C(3) = front right
  pinMode((uint32_t)51, OUTPUT); //Motor D(4) = back right
...
void USResponse(){
  ...
  //testing
  Serial.print("ultrasonic distance measure(cm):");
  Serial.println(dist);
  /**
  if (dist <= 20.0){
    stop();
...
void TOFResponse(){
  //testing
  int rightSensor = sensors[0].read();
  int leftSensor = sensors[1].read();
  Serial.print("Left side: (cm) ");
  Serial.println(rightSensor/10);
  Serial.print("right side: (m) ");
  Serial.println(leftSensor/10);
  /**
